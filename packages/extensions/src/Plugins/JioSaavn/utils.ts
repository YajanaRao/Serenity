...
export async function parseSongs(data: any) {
  // const songs = [];
  const songs = await Promise.all(data.songs.data.map(async (item: any) => {
...
export async function parseCollection(data: any) {
...
export function formatSong(data: any) {
  // try {
