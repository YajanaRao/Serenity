import { ADD_SONG, TOGGLE_TODO } from '../actions/actionTypes';
  ...
  songs: [
    {
      ...
      played: false,
      image:
        'https://images.unsplash.com/photo-1562872458-d92517741af5?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1456&q=80',
...
function songs(state: SONG[] = [], action) {
  switch (action.type) {
    case ADD_SONG:
      return [...state, action.song];
    case TOGGLE_TODO:
      return state.map((song: SONG, index) => {
...
export function libraryManager(state = initialState, action) {
  switch (action.type) {
    case ADD_SONG:
